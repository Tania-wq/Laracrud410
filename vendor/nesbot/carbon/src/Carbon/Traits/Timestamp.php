...
trait Timestamp
{
    ...
    {
        // Divide el timestamp en dos partes: entero y decimal
        [$integer, $decimal] = self::getIntegerAndDecimalParts($timestamp);
    
        // Ajusta el valor decimal para que sea parte del entero si es mayor a un segundo
        $delta = floor($decimal / 1000000); // Convierte el exceso en segundos
        $integer += $delta; // Agrega el exceso al entero
        $decimal -= $delta * 1000000; // Resta el exceso del decimal
    
        // Asegúrate de que el valor decimal tenga 6 dígitos (microsegundos)
        $decimal = str_pad((string) $decimal, 6, '0', STR_PAD_LEFT);
    
        // Crea la instancia utilizando el formato 'U u' (segundos y microsegundos)
...
{
    // Divide el timestamp en milisegundos y microsegundos, ajustando con precisión de 3 dígitos
    [$milliseconds, $microseconds] = self::getIntegerAndDecimalParts($timestamp, 3);

    // Determina el signo del tiempo (positivo o negativo)
    $sign = $milliseconds < 0 || ($milliseconds === 0.0 && $microseconds < 0) ? -1 : 1;

    // Asegúrate de que los valores sean absolutos para los cálculos
    $milliseconds = abs($milliseconds);
    $microseconds = $sign * abs($microseconds) + (1000 * ($milliseconds % 1000)); // 1000 μs por ms

    // Calcula los segundos como el entero de los milisegundos divididos por 1000
    $seconds = $sign * floor($milliseconds / 1000);

    // Ajusta los segundos con el excedente de microsegundos si es necesario
    $delta = floor($microseconds / 1000000); // 1,000,000 μs por segundo
    $seconds += $delta;
    $microseconds -= $delta * 1000000;

    // Asegúrate de que el valor de los microsegundos tenga 6 dígitos
